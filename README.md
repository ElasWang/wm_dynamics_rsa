...
- H1：逐时间点模型拟合差分 → **cluster-based permutation test**
- H2：条件级 RDM 上三角多元回归，β 与 ΔR² 单样本单侧 t 检验
  - M1：Content（字母身份编码）
  - M2：Content + Visual（增加视觉特征）
  - M3：Content + Visual + Task（增加任务 / 记忆负载特征）
  - M4：Full = M3 + Structure（加入序列结构模型）
- H3：训练窗口（编码期）→ 测试窗口（维持期）泛化矩阵，SSI = 结构拟合 − 控制模型平均拟合，单样本 t 检验 + 结构 vs 各控制模型配对 t 检验
...
**结论：** 条件级支持，事件级与源空间不稳健。 在控制内容、视觉、颜色后，结构模型仅在条件级（_cv）表现出显著独特贡献。
...
- **时间自相关未完全排除：** H1 中 gradient > timestamp 在所有粒度均无显著簇；H3 中结构泛化未显著优于时间戳模型。目前无法完全排除“结构效应来自 EEG 普遍时间自相关”的替代解释。
...
- **H2 仅在条件级成立：** 结构独特贡献在条件级（_cv）显著，但在事件级（_event）与源空间（_source_cv）不稳健。源空间 ΔR² 仅边缘显著（p = 0.068），提示预测精度提升有限。
- **H3 结构特异性有限：** SSI 在条件级与源空间显著 > 0，且结构泛化显著优于视觉与颜色；但 vs 时间模型在所有粒度均不显著（p > 0.63），无法排除时间自相关。
- **条件级刺激前假阳性：** 条件级（_cv）在 0–68 ms 出现显著簇，可能为假阳性或基线噪声，需谨慎解读，必要时在正式统计中剔除刺激前窗口。
---
...
| # | 任务 | 目的                                                 |
|---|---|----------------------------------------------------|
| 1 | **H3 行为关联收尾**：SSI × RT/ACC 被试层面相关 （行为表已构建，N=21） | 完成**结构稳定性 ↔ 行为效率**证据链                              |
| 2 | **时间自相关证伪**：temporal-distance matched control / shuffled null | 排除**泛化优势来自 EEG 普遍时间自相关**的替代解释，确认 structure-specific |
| 3 | **H2 稳健性**：模型共线性诊断（VIF / 正交化） | 解释独特贡献为何仅在条件级成立              |
| 4 | **源空间 ROI 分析**：DMN / FPN / Visual 三 ROI 的时间动态 RSA | 从头皮表征定位到脑区，排除体积传导                                  |
| 5 | **动态加权混合模型**：扫描 α, λ 组合，输出 α(t) 结构-内容权重漂移曲线 | 机制化探索：表征资源如何随时间动态调配                                |
...
若 RNN 延迟期 RDM 与真实 EEG 延迟期 RDM 显著相关、且该相关**仅出现在额叶**（对应 RNN 决策单元）而非视觉电极，即构成"机制层面"的强证据：**前额叶的循环动态（持续性放电）承载了工作记忆的结构表征，并决定了行为表现**。
